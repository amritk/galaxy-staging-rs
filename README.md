...
This library provides convenient access to the Demo API (Scalar Galaxy) from asynchronous Rust.
...
```toml
[dependencies]
amritk-galaxy-staging = "0.1.0" # x-release-please-version
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let client = DemoApiScalarGalaxyClient::builder()
        .bearer_auth(std::env::var("BEARER_AUTH")?)
        .build()?;
...
- `o_auth2` — environment variable `O_AUTH2`
- `open_id_connect` — environment variable `OPEN_ID_CONNECT`
- `client_id` — environment variable `OFFICIAL-GALAXY-TESTING_CLIENT_ID`
- `client_secret` — environment variable `OFFICIAL-GALAXY-TESTING_CLIENT_SECRET`
...
`o_auth2` explicitly skips the exchange entirely.
...
`OFFICIAL-GALAXY-TESTING_OAUTH_SCOPES` environment variable, whitespace-separated) —
asking for a scope that was not granted is rejected as `invalid_scope`:

```rust,ignore
let client = DemoApiScalarGalaxyClient::builder().oauth_scopes(["read:account"]).build()?;
```
...
Construct one `DemoApiScalarGalaxyClient` and share it for the lifetime of your
